...
public class AppointmentBook {
	...
	/*
	 * Returns the arrayList containing the appointments
	 */
	...
	public void add(AppointmentType type, String desc, GregorianCalendar date){
		...
		else if(type==AppointmentType.ONETIME){
	...
	public static void save(String file, Appointment appointment) throws IOException{
		...
		if(appointment.getType()==AppointmentType.DAILY)
			PW.print("DAILY");
		else if(appointment.getType()==AppointmentType.MONTHLY)
			PW.print("MONTHLY");
		else if(appointment.getType()== AppointmentType.ONETIME)
			PW.print("ONETIME");
	...
	public void load(String file) throws FileNotFoundException{
		...
		in.useDelimiter("~|\r\n");
		while(in.hasNext()){
			String checkEmpty = in.next();
			if(!(checkEmpty.isEmpty())){
				String typeString = checkEmpty;
				...
				else if(typeString.equals("MONTHLY"))
					add(AppointmentType.MONTHLY, description, date);
				else if(typeString.equals("ONETIME"))
