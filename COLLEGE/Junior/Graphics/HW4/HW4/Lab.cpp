...
std::shared_ptr<SpotLight> light2 = std::make_shared<SpotLight>(glm::vec3(0, 10, 0), FOV, glm::vec3(0, 1, 0));
...
void buildScene() {
	...
	PhongMaterial bronze(color(0.2125f, 0.1275f, 0.054f, 1.0f), color(0.393548, 0.271906, 0.166721, 1), color(0.714, 0.4284, 0.18144, 1), 25.6);
	PhongMaterial gold(color(0.24725, 0.1995, 0.0745, 1), color(0.628281, 0.555802, 0.366065, 1), color(0.75164, 0.60648, 0.22648, 1), 51.2);
	PhongMaterial redPlastic(color(0.1, 0.1, 0.1, 1), color(0.7, 0.6, 0.6, 1), color(0.5, 0, 0, 1), 32);
	PhongMaterial silver(color(0.19225, 0.19225, 0.19225, 1), color(0.50754, 0.50754, 0.50754, 1), color(0.508273, 0.508273, 0.508273, 1), 51.2);

	// Add light sources to vector of light sources
	light1->attenParams = attenuationParams[0];
	light2->attenParams = attenuationParams[1];
	light1->isOn = lightIsOn[0];
	light2->isOn = false;
	light1->attenIsOn = attenuationIsOn[0];
	light2->attenIsOn = attenuationIsOn[1];
	lights.push_back(light1);
	lights.push_back(light2);

	// Create objects in the scene
	std::shared_ptr<Plane> plane = std::make_shared <Plane>(glm::vec3(0.0f, -4.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f), color(0,0,0,1), bronze);

	QuadricParameters sphereParams = { 1.0f, 1.0f, 1.0f, 0, 0, 0, 0, 0, 0, -4 };
	std::shared_ptr<QuadricSurface> sphere = std::make_shared<QuadricSurface>(sphereParams, glm::vec3(-6.0f, -2.0f, -2.0f), color(0,0,0,1), gold);
	std::shared_ptr<CylinderY> cylinder = std::make_shared<CylinderY>(glm::vec3(5.0f, -3.0f, 0.0f), 3.0f, 2.0f, color(0,0,0,1), silver);
	QuadricParameters hyperParams = { 1.0f, 0.0f, 1.0f, 0, 0, 0, 0, 3, 0, 0 };
	std::shared_ptr<QuadricSurface> paraboloid = std::make_shared<QuadricSurface>(hyperParams, glm::vec3(0.0f, 3.0f, -5.0f), color(0,0,0,1), redPlastic);
...
void PrintLightInfo() {
	...
	std::cout << "Is on: " << (light1->isOn ? "true" : "false") << std::endl;
	std::cout << "Attenuation is on: " << (light1->attenIsOn ? "true" : "false") << std::endl;
	std::cout << std::endl;
	std::cout << "Light #2" << std::endl;
	std::cout << *((SpotLight*)(lights[1].get())) << std::endl;
	std::cout << "Is on: " << (light2->isOn ? "true" : "false") << std::endl;
	std::cout << "Attenuation is on: " << (light2->attenIsOn ? "true" : "false") << std::endl;
	std::cout << "FOV: " << glm::round(glm::degrees(light2->FOV)) << std::endl ;
...
void KeyboardCB(unsigned char key, int x, int y) {
	...
	case 'A':
	case 'a':	lights[currLight]->attenIsOn = !(lights[currLight]->attenIsOn);  break;
	case '1':	currLight = 0; break;
	case '2':	currLight = 1; break;
	case 'w':	sl = (SpotLight*)(lights[1].get());
				incrementClamp(sl->FOV, -3.14f / 36, 0, 1.5f);
				break;
	case 'W':	sl = (SpotLight*)(lights[1].get());
				incrementClamp(sl->FOV, 3.14f/36, 0, 1.5f);
				break;
	case 'c':	incrementClamp(lights[currLight]->attenParams.x, -INC, 0.0f, 10.0f); break;
		lights[currLight]->attenParams = attenuationParams[currLight];
