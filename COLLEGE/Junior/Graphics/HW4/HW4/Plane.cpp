...
*/
Plane::Plane(const glm::vec3 & point, const glm::vec3 & normal, const Material &material, const PhongMaterial & mat)
	: Surface(material), a(point), n(normalize(normal)){
	phongMat = mat;
}

Plane::Plane(std::vector<glm::vec3> vertices, const Material & material, const PhongMaterial & mat)
	: Surface(material) {
	a = vertices[0];
	phongMat = mat;
