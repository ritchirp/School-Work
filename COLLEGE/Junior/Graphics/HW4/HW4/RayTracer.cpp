...
color RayTracer::traceIndividualRay(const glm::vec3 &e, const glm::vec3 &d,
	...
	for (unsigned int i = 0; i < surfacesInScene.size(); i++) {
		...
		if (thisHit.t != FLT_MAX && thisHit.t < theHit.t) {
			theHit = thisHit;
			mat = surfacesInScene[i] -> phongMat;
		}
	}
	if (theHit.t == FLT_MAX) {
		return defaultColor;
	}
	color diffuse = color(0, 0, 0, 1);
	color spec = color(0, 0, 0, 1);
	for (unsigned int i = 0; i < lightsInScene.size(); i++) {
		...
		color temp = lightsInScene[i]->illuminate(theHit, d, r.t < dist);
		
		// Calculations
		int shadowFactor = r.t < dist ? 0 : 1;
		float attenFactor = (lightsInScene[i]->attenIsOn ? lightsInScene[i]->attenFactor(dist) : 1);
		float ln = glm::dot(shadowFeelerDirection, glm::normalize(theHit.surfaceNormal));
		glm::vec3 refl = glm::normalize((2 * ln * theHit.surfaceNormal) - shadowFeelerDirection);
		glm::vec3 v = glm::normalize(e - theHit.interceptPoint);

		// Add to the local illumination components
		spec += attenFactor * shadowFactor * pow(glm::max(0.0f, glm::dot(refl, v)) , mat.shininess) * temp;
		diffuse += attenFactor * shadowFactor * glm::max(0.0f, ln) * temp;

	}

	return glm::clamp(mat.ambientColor * color(1,1,1,1) + mat.diffuseColor * diffuse + spec * mat.specularColor, 0.0f , 1.0f);
