...
RayTracer rayTrace1(frameBuffer, color(0.9, 0.9, 0.9, 1));
RayTracer rayTrace2(frameBuffer, color(0.9, 0.9, 0.9, 1));
...
//	frameBuffer.clearColorAndDepthBuffers(); // Not necessary for ray tracing
	...
	rayTrace1.raytraceScene1(surfaces, lights);
	rayTrace2.raytraceScene2(surfaces, lights);
...
static void ResizeCB(int width, int height) {
	...
	rayTrace1.setCameraFrame(glm::vec3(0, 10, 10), glm::vec3(0, -10, -10), glm::vec3(0, 1, 0));
	rayTrace1.calculatePerspectiveViewingParameters(45.0f);
	rayTrace2.setCameraFrame(glm::vec3(-10, 0, 5), glm::vec3(10, 0, -5), glm::vec3(0, 1, 0));
	rayTrace2.calculatePerspectiveViewingParameters(45.0f);
...
static void KeyboardCB(unsigned char key, int x, int y) {
	...
	case(27): // Escape key
		...
		break;
	case('1'):
		rayTrace1.setAntiAliasingLevel(1);
		rayTrace2.setAntiAliasingLevel(1);
		break;
	case('2'):
		rayTrace1.setAntiAliasingLevel(2);
		rayTrace2.setAntiAliasingLevel(2);
		break;
	case('3'):
		rayTrace1.setAntiAliasingLevel(3);
		rayTrace2.setAntiAliasingLevel(3);
		break;
	default:
...
void buildScene() {
	...
	std::shared_ptr<Disk> redDisk = std::make_shared<Disk>(glm::vec3(-8.0f, 0.0f, -14.0f), glm::vec3(0.0f, 0.0f, -1.0f), 1.0f, color(1.0f, 0.0f, 0.0f, 1.0f));
	...
	QuadricParameters qParams = { 1.0f, 1.0f, 1.0f, 0, 0, 0, 0, 0, 0, -4 };
	std::shared_ptr<QuadricSurface> magentaQuadric = std::make_shared<QuadricSurface>(qParams, glm::vec3(2.0f, 0.0f, 0.0f), color(1.0f, 0.0f, 1.0f, 1.0f));
	...
	rayTrace1.setLight(PositionalLight(glm::vec3(-10.0f, 4.0f, 0.0f), glm::vec4(.75, .75, .75, 1)));
	rayTrace2.setLight(PositionalLight(glm::vec3(-10.0f, 4.0f, 0.0f), glm::vec4(.75, .75, .75, 1)));
...
int main(int argc, char** argv) {
	...
	rayTrace1.setDefaultColor(color(0.9, 0.9, 0.9, 1));
